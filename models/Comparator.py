...
class Comparator_v1(Module):
    def __init__(self, conv_dim, image_size):
        ...
        self.conv2 = Sequential(
            ConvBlock(curr_dim, curr_dim, norm='', act='lrelu', sn=True, kernel_size=3, stride=1, padding=1),
            ConvBlock(curr_dim, conv_dim, norm='', act='lrelu', sn=True, kernel_size=4, stride=2, padding=1))
        ...
        self.out_conv = ConvBlock(curr_dim, 1024, kernel_size=4, stride=1, padding=0, bias=False)           
    ...
    def forward(self, img1, img2):
        ...
        feat1 = torch.cat([img1, self.stn(self.conv_layers[0](img1))], dim=1)
        for i in range(1, len(self.conv_layers)):
            feat1 = torch.cat([F.avg_pool2d(feat1,2), self.conv_layers[i](feat1)], dim=1)

        feat2 = torch.cat([img2, self.stn(self.conv_layers[0](img2))], dim=1)
        for i in range(1, len(self.conv_layers)):
            feat2 = torch.cat([F.avg_pool2d(feat2,2), self.conv_layers[i](feat2)], dim=1)
...
class STN_comp(STN):
    def __init__(self, in_dim):
        ...
        self.conv = Sequential(
            ConvBlock(in_dim, in_dim, norm='in', act='relu', kernel_size=4, stride=2, padding=1),
