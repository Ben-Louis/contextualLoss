...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    with tf.variable_scope("g") as scope:
        generator = recursive_generator(input_image_A, config.TRAIN.sp) # sp=256
        scope.reuse_variables()
        generator_test = recursive_generator(input_A_test, config.TRAIN.sp) # 所以说这个模型是对每个style的图片都要训练很久吗？？？？


    weight = tf.placeholder(tf.float32) # 这个好像后面就没有用到了
    ...
    if config.W.CX_content > 0:  # default: 1.0
        CX_loss_content_list = [w * CX_loss_helper(vgg_input[layer], vgg_fake[layer], config.CX)
                                for layer, w in config.CX.feat_content_layers.items()]  # w都是0
